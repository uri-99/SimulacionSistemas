...
class Galaxy:
 ...
 #  Planet = Object(radius, mass, coords, speed, initT, dt)   #km, kg, [posX, posY], [speedX, speedY], t, dt 
    Earth = Object(6371, 5.97*10**24, earth_initial_pos, earth_initial_speed, 1.99106393e-7, t, dt)
    Mars = Object(3390, 6.4171*10**23, mars_initial_pos, mars_initial_speed, 1.05865302e-7, t, dt)
    ...
    def __init__(self, shipCoords):
        self.Ship = SpaceShip(2*10**5, shipCoords, 7.12, self.t, self.dt, self.system) #1500km de la superficie y 7.12 km/s
    ...
    def shipTimeToTakeOff(self):
        ship_angle = self.Ship.angle_to_sun()
        earth_angle = self.Earth.angle_to_sun()

        if(math.fabs(ship_angle - earth_angle) >= self.alignment_condition):
            return True
        return False