...
simulationFile = os.path.join(dirname, 'simulationData')
...
for i in range (m):
    ...
    counter += l/m
axs.set_xticks(xarray)
axs.set_yticks(xarray)


plt.grid(markevery = 1, c="black", linewidth=0.5)
...
def on_click(event):
    ...
    if (selectedParticle):
        ...
        rcCircle.append(
            plt.Circle((selectedParticle.x, selectedParticle.y), selectedParticle.r + rc, color='yellow', fill=False))
        if cimp[0] == "true":
            ...
            if (posX > l):
                rcCircle.append(
                    plt.Circle((selectedParticle.x - l, selectedParticle.y), selectedParticle.r + rc, color='yellow',
                               fill=False))
            if (minusX < 0):
                rcCircle.append(
                    plt.Circle((selectedParticle.x + l, selectedParticle.y), selectedParticle.r + rc, color='yellow',
            ...
            if (posY > l):
                rcCircle.append(
                    plt.Circle((selectedParticle.x, selectedParticle.y - l), selectedParticle.r + rc, color='yellow',
                               fill=False))
            if (minusY < 0):
                rcCircle.append(
                    plt.Circle((selectedParticle.x, selectedParticle.y + l), selectedParticle.r + rc, color='yellow',
            ...
            if (posX > l and posY > l):
                rcCircle.append(plt.Circle((selectedParticle.x - l, selectedParticle.y - l), selectedParticle.r + rc,
